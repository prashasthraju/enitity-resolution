...
def featurize_candidates(cands: pd.DataFrame, s1: pd.DataFrame, other: pd.DataFrame, n_jobs: int = 1) -> pd.DataFrame:
    ...
    feat_df = pd.DataFrame(rows, columns=FEATURE_NAMES, dtype=np.float32)
...
def build_labeled_set(cands_feat: pd.DataFrame, gt: pd.DataFrame, negative_sample_rate: float = 0.1) -> pd.DataFrame:
    gt_pairs = set()
    ...
    df = cands_feat.copy()
    df["label"] = np.array([(a, b) in gt_pairs for a, b in zip(df["source1_entity_id"], df["candidate_id"])], dtype=np.int8)
    ...
    if negative_sample_rate < 1.0:
        pos = df[df["label"] == 1]
        neg = df[df["label"] == 0].sample(frac=negative_sample_rate, random_state=42)
        df = pd.concat([pos, neg], ignore_index=True).sample(frac=1.0, random_state=42)
        print(f"[memory] Downsampled negatives. Training rows: {len(df)}")
...
def main():
    ...
    labeled_tr = build_labeled_set(feat_tr, gt_tr, negative_sample_rate=0.1)
    del feat_tr
    gc.collect()
    ...
    feat_va["score"] = model.predict_proba(feat_va[FEATURE_NAMES].astype(np.float32))[:, 1]
    best_t, best_f05 = tune_threshold(feat_va, gt_va, s1_va["entity_id"])
    del feat_va, other_all
    gc.collect()
    ...
    other_all_train = pd.concat([train_s2, train_s3], ignore_index=True)
    
    feat_full = featurize_candidates(cand_full, train_s1, other_all_train, n_jobs=args.n_jobs)
    del cand_full, other_all_train
    gc.collect()
    
    labeled_full = build_labeled_set(feat_full, gt, negative_sample_rate=0.1)
    del feat_full
    gc.collect()
    ...
    del cand_test, test_other
    gc.collect()
    
    feat_test["score"] = final_model.predict_proba(feat_test[FEATURE_NAMES].astype(np.float32))[:, 1]
