...
def preprocess_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Addresses.
    addr_parts = out.apply(
        lambda r: normalize_address_with_commas(r["business_address"], r["country"]), axis=1
    )
...
def open_db(path: Path) -> sqlite3.Connection:
    ...
    con.execute("PRAGMA cache_size=-200000")  # ~200MB when supported.
    return con
...
def insert_records(con: sqlite3.Connection, df: pd.DataFrame, source: str, chunk_name: str = "") -> None:
    ...
    rows = [tuple(row[c] for c in cols) + (source,) for row in df.to_dict("records")]
    con.executemany(sql, rows)
    con.commit()
    log(f"Inserted {len(rows):,} {source} rows {chunk_name}".strip())


def load_sources_into_db(con: sqlite3.Connection, data_dir: Path, prefix: str, chunk_size: int = 100_000) -> pd.DataFrame:
...
def feature_frame(rows: pd.DataFrame) -> pd.DataFrame:
    ...
    def ratio_arr(a: np.ndarray, b: np.ndarray) -> np.ndarray:
        return np.fromiter((fuzz.ratio(x, y) for x, y in zip(a, b)), dtype=np.float32, count=len(a)) / 100.0

    def token_ratio_arr(a: np.ndarray, b: np.ndarray) -> np.ndarray:
        return np.fromiter((fuzz.token_set_ratio(x, y) for x, y in zip(a, b)), dtype=np.float32, count=len(a)) / 100.0
...
def collect_training_arrays(con: sqlite3.Connection, max_rows: int = 6_000_000) -> Tuple[np.ndarray, np.ndarray]:
    xs, ys = [], []
    seen = 0
    for df in stream_pair_features(con, "train_pairs", chunk_size=80_000, with_labels=False):
        X = feature_frame(df)
...
def validation_arrays(con: sqlite3.Connection, model: lgb.LGBMClassifier, chunk_size: int = 80_000) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
...
def write_predictions(con: sqlite3.Connection, pred_path: Path, threshold: float, model: lgb.LGBMClassifier, chunk_size: int = 80_000) -> None:
...
def build_train_db(train_dir: Path, work_dir: Path) -> Path:
    db_path = work_dir / "train_er.sqlite"
    con = open_db(db_path)
    init_record_db(con)
    s1_ids_df = load_sources_into_db(con, train_dir, "train")
    load_ground_truth(con, train_dir / "train_ground_truth.tsv", s1_ids_df["entity_id"].tolist())
    generate_candidates(con)
    materialize_train_pairs(con, max_pos=2_500_000, max_neg=4_000_000, singleton_neg=1_000_000)
    con.close()
    return db_path


def run_train(train_db: Path, model_path: Path) -> Tuple[lgb.LGBMClassifier, float, float]:
    con = sqlite3.connect(str(train_db))
    X, y = collect_training_arrays(con, max_rows=7_000_000)
...
def test_db(test_dir: Path, work_dir: Path) -> Path:
    db_path = work_dir / "test_er.sqlite"
    con = open_db(db_path)
    init_record_db(con)
    load_sources_into_db(con, test_dir, "test")
...
def main() -> int:
    ...
    t0 = time.time()
    train_db = build_train_db(args.train_dir, args.work_dir)
    model, threshold, val_f = run_train(train_db, args.model)
    log(f"Training/validation completed in {(time.time()-t0)/60:.1f} min; threshold={threshold:.4f}, validation F0.5={val_f:.6f}")

    test_db_path = test_db(args.test_dir, args.work_dir)
